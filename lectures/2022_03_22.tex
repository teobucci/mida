...
First we select a parametric model class $\mathcal{M}(\theta)$ ($\theta$= parameters vector – each different $\theta$ corresponds to a different model).

For example:
$$
	\mathcal{M}=\left\{y(t)=\frac{B(z, \theta)}{A(z, \theta)} u(t-d)+\frac{C(z, \theta)}{A(z, \theta)} e(t), \quad e(t) \sim WN\left(0, \lambda^{2}\right), \theta\in\Theta\right\}
$$
...
\emph{Example.}
$$
	M(\theta): y(t)=\frac{b+b^{2} z^{-1}}{1-a z^{-1}} u(t-d)+\frac{1+a z^{-1}}{1-a z^{-1}} e(t)
$$
Here, the parameter vector is given by $\theta=\begin{bmatrix}a \\ b\end{bmatrix}$ only.
...
\boxedText{For every $\theta \in \Theta$, the stochastic part of $M(\theta)$ (i.e. the part depending on the white noise $\left.e(t) \sim W N\left(0, \lambda^{2}\right)\right)$ is \textbf{canonical} and has \textbf{no zeroes on the unit circle}.}
...
$M(\theta)\implies \hat{M}(\theta)$ from stochastic models to predictive models.

ARMAX in $\mathcal{M}$ for a given value of $\theta$:

\begin{align*}
	M(\theta)&:\quad y(t)=\frac{B(z, \theta)}{A(z, \theta)} u(t-d)+\frac{C(z, \theta)}{A(z, \theta)} e(t)\\
	&\qquad\Downarrow\\
	\hat{M}(\theta)&: \quad \hat{y}(t \mid t-1) =\frac{B(z,\theta) E(z,\theta)}{C(z,\theta)} u(t-d)+\frac{F(z,\theta)}{C(z,\theta)} y(t-1) \qquad \text{one step prediction}
\end{align*}
